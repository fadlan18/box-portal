  ...
  <div>
    ...
    <div v-else-if="invoice && template === 'professional'" id="invoice-print"
      ...
      <div style="max-width:800px;margin:0 auto;background:white;box-shadow:0 2px 16px rgba(0,0,0,0.1);position:relative;overflow:hidden">
        ...
        <!-- Pita status — clip-path agar tidak terpotong di sudut -->
        <div style="position:absolute;top:0;right:0;width:260px;height:260px;z-index:10;pointer-events:none;clip-path:polygon(100% 0, 100% 100%, 0 0);print-color-adjust:exact;-webkit-print-color-adjust:exact">
          <div :style="'position:absolute;width:380px;top:120px;right:-60px;text-align:center;transform:rotate(35deg);transform-origin:center center;print-color-adjust:exact;-webkit-print-color-adjust:exact;' + (invoice.status === 'paid' ? 'background:#10b981' : 'background:#ef4444')"
